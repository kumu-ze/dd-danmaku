...
(async function () {
    ...
    const Config = {
        // Basic settings
        CHECK_INTERVAL: 200,
        CH_CONVERT_TITLES: ['当前状态: 未启用', '当前状态: 转换为简体', '当前状态: 转换为繁体'],

        // Icon constants (0: off, 1: on)
        ICONS: {
            DANMAKU: ['\uE7A2', '\uE0B9'],
            SEARCH: '\uE881',
            TRANSLATE: '\uE927',
        ...
        STYLES: {
            ...
            RANGE_SLIDER: { class: 'emby-slider emby-slider-scalebg emby-slider-nothumb', is: 'emby-slider' },
            SLIDER_CONTAINER: { class: 'slidercontainer flex-grow emby-slider-container' },
            SLIDER_WRAPPER: { class: 'videoOsdVolumeSliderWrapper flex-grow' },
            SLIDER_DIV: {
        ...
        SELECTORS: {
            UI_ANCHOR: '\uE034',
            MEDIA_CONTAINER: 'div[class~="view-videoosd-videoosd"]',
            MEDIA: 'video',
        },
        ...
        DEFAULT_PROXY_SERVERS: ['https://www.kumuze-dd.icu/'],
    };
    ...
    const chConverTtitle = Config.CH_CONVERT_TITLES;
    const danmaku_icons = Config.ICONS.DANMAKU;
    const search_icon = Config.ICONS.SEARCH;
    const translate_icon = Config.ICONS.TRANSLATE;
    const filter_icons = Config.ICONS.FILTER;
    const transparency_icons = Config.ICONS.TRANSPARENCY;
    const info_switch_icons = Config.ICONS.INFO_SWITCH;
    const more_filter_icon = Config.ICONS.MORE_FILTER;
    const log_icon = Config.ICONS.LOG;
    const settings_icon = Config.ICONS.SETTINGS;
    const reset_icon = Config.ICONS.RESET;
    const menubarOptions = Config.STYLES.MENUBAR;
    const buttonOptions = Config.STYLES.BUTTON;
    const rangeSliderOptions = Config.STYLES.RANGE_SLIDER;
    const sliderContainerOptions = Config.STYLES.SLIDER_CONTAINER;
    const sliderWrapperOptions = Config.STYLES.SLIDER_WRAPPER;
    const sliderdivOptions = Config.STYLES.SLIDER_DIV;
    const sliderLabelOptions = Config.STYLES.SLIDER_LABEL;
    const uiAnchorStr = Config.SELECTORS.UI_ANCHOR;
    const mediaContainerQueryStr = Config.SELECTORS.MEDIA_CONTAINER;
    const mediaQueryStr = Config.SELECTORS.MEDIA;
    const fontSizeDesktop = Config.DEVICE.FONT_SIZE_DESKTOP;
    const fontSizeMobile = Config.DEVICE.FONT_SIZE_MOBILE;
    const defaultProxyServers = Config.DEFAULT_PROXY_SERVERS;

    // Utility Functions Module
    const Utils = {
        // DOM query cache for performance
        _cache: new Map(),
        ...
        // Show toast notification with debouncing
        showToast: null, // Will be initialized below
    };

    // Initialize debounced showToast
    Utils.showToast = Utils.debounce(function (message, type = 'info') {
    ...
    const StorageManager = {
        ...
        // Set value to localStorage with JSON serialization
        set(key, value) {
            try {
    ...
    // Button Factory with optimized configurations
    const ButtonFactory = {
        // Check if loading with early return
        checkLoading() {
            if (window.ede.loading) {
                Utils.showToast('正在加载,请稍后再试', 'info');
                return true;
            }
            return false;
        },
        ...
        // Update button icon
        updateButtonIcon(id, iconText) {
            const button = Utils.$(`#${id}`);
            if (button) {
        ...
        configs: {
            displayDanmaku: {
                ...
                innerText: null,
                onclick: () => {
                    if (ButtonFactory.checkLoading()) return;

                    console.log('切换弹幕开关');
                    window.ede.danmakuSwitch = (window.ede.danmakuSwitch + 1) % 2;
                    StorageManager.set('danmakuSwitch', window.ede.danmakuSwitch);
                    ButtonFactory.updateButtonIcon('displayDanmaku', danmaku_icons[window.ede.danmakuSwitch]);
            ...
            translateDanmaku: {
                ...
                onclick: () => {
                    ...
                    const button = Utils.$('#translateDanmaku');
                    if (button) button.setAttribute('title', chConverTtitle[window.ede.chConvert]);
            ...
            filteringDanmaku: {
                ...
                onclick: () => {
                    ...
                    ButtonFactory.updateButtonIcon('filteringDanmaku', filter_icons[level]);
                    reloadDanmaku('reload');
                },
            },
            ...
            switchDanmakuInfo: {
                ...
                onclick: () => {
                    ...
                    ButtonFactory.updateButtonIcon('switchDanmakuInfo', info_switch_icons[window.ede.showDanmakuInfo ? 1 : 0]);

                    const infoElement = Utils.$('#videoOsdDanmakuTitle');
                    if (infoElement) {
            ...
            danmakuSettings: {
                ...
                onclick: () => showDanmakuSettingsDialog(),
            },
        },
    };

    // Legacy button configurations for compatibility
    const displayButtonOpts = ButtonFactory.configs.displayDanmaku;
    const searchButtonOpts = ButtonFactory.configs.searchDanmaku;
    const translateButtonOpts = ButtonFactory.configs.translateDanmaku;
    const filterButtonOpts = ButtonFactory.configs.filteringDanmaku;
    const infoSwitchButtonOpts = ButtonFactory.configs.switchDanmakuInfo;
    const settingsButtonOpts = ButtonFactory.configs.danmakuSettings;
    ...
    // Optimized EDE class with better initialization and error handling
    class EDE {
        constructor() {
            // Initialize with defaults and load from storage
        ...
        initializeProperties() {
            ...
            this.showDanmakuInfo = true;
            this.cacheEnabled = true;
            this.currentProxyIndex = 0;
            this.customProxyServer = '';
            this.filterWords = [];
            this.buttonOrder = ['displayDanmaku', 'danmakuSettings', 'filterSettings', 'switchDanmakuInfo', 'searchDanmaku', 'showDanmakuLog'];
        }

        loadFromStorage() {
            try {
                // Load settings from storage with defaults
                this.chConvert = StorageManager.get('chConvert', this.chConvert);
                this.danmakuSwitch = parseInt(StorageManager.get('danmakuSwitch', this.danmakuSwitch));
                this.showDanmakuInfo = StorageManager.get('showDanmakuInfo', this.showDanmakuInfo);
                this.cacheEnabled = StorageManager.get('danmakuCacheEnabled', this.cacheEnabled);
                this.filterWords = StorageManager.get('danmakuFilterWords', this.filterWords);
                this.buttonOrder = StorageManager.get('danmakuButtonOrder', this.buttonOrder);
                this.customProxyServer = StorageManager.get('danmakuCustomProxy', this.customProxyServer);
                this.currentProxyIndex = parseInt(StorageManager.get('danmakuProxyIndex', this.currentProxyIndex));
            } catch (error) {
        ...
        initializeEventHandlers() {
            // Add cleanup handlers for better memory management
        ...
        cleanup() {
            ...
            if (this.danmaku) {
                try {
                    ...
                    console.error('Error destroying danmaku:', error);
                }
                this.danmaku = null;
            }
        ...
        saveSettings() {
            const settings = {
                ...
                showDanmakuInfo: this.showDanmakuInfo,
                cacheEnabled: this.cacheEnabled,
                filterWords: this.filterWords,
                buttonOrder: this.buttonOrder,
                customProxyServer: this.customProxyServer,
                currentProxyIndex: this.currentProxyIndex,
            };

            Object.entries(settings).forEach(([key, value]) => {
                const storageKey = key === 'chConvert' || key === 'danmakuSwitch' || key === 'showDanmakuInfo' ? key : `danmaku${key.charAt(0).toUpperCase() + key.slice(1)}`;
    ...
    const ComponentFactory = {
        // Create range slider with improved error handling
        createRangeSlider(opt, button) {
            try {
                const input = Utils.createElement('input', {
                    ...
                    max: opt.max,
                    value: opt.value,
                    class: rangeSliderOptions.class,
                    is: rangeSliderOptions.is,
                });

                if (input && opt.oninput) {
                    input.oninput = opt.oninput;
                }

                const sliderContainer = Utils.createElement('div', {
                    class: sliderContainerOptions.class,
                });

                const sliderWrapper = Utils.createElement('div', {
                    class: sliderWrapperOptions.class,
                });

                const sliderDiv = Utils.createElement('div', {
                    class: sliderdivOptions.class,
                    style: sliderdivOptions.style,
                });

                if (input && sliderContainer && sliderWrapper && sliderDiv) {
                ...
                console.error('Failed to create range slider components');
                return null;
            } catch (error) {
                console.error('Error creating range slider:', error);
                return null;
            }
        },

        // Create button with improved error handling
        createButton(opt) {
            try {
                ...
                if (button && icon) {
                    ...
                    if (opt.onclick) button.onclick = opt.onclick;
                    return button;
                }

                console.error('Failed to create button components');
                return null;
            } catch (error) {
    ...
    function createRangeSlider(opt, button) {
        return ComponentFactory.createRangeSlider(opt, button);
    }

    function createButton(opt) {
        return ComponentFactory.createButton(opt);
    }

    // Optimized listener initialization with debouncing
    const initListener = Utils.debounce(function () {
        const container = Utils.$(mediaQueryStr, false); // Don't cache media elements
    ...
    // Optimized element search function
    const getElementsByInnerText = Utils.debounce(function (tagType, innerStr, excludeChildNode = true) {
        try {
            ...
            const matching = Array.from(elements).filter((e) => e.innerText && e.innerText.includes(innerStr));

            if (!excludeChildNode) {
                return matching;
            }

            // Filter out elements where the text is only in child nodes
            return matching.filter((e) => {
                const clone = e.cloneNode(true);
                while (clone.firstChild !== clone.lastChild && clone.lastChild) {
                    clone.removeChild(clone.lastChild);
                }
                return clone.innerText && clone.innerText.includes(innerStr);
            });
        } catch (error) {
            console.error('Error in getElementsByInnerText:', error);
            return [];
        }
    }, 50);
    ...
    async function getEpisodeInfo(is_auto = true) {
        try {
            ...
            if (lastSearch) {
                ...
                if (now - lastSearchData.timestamp < 60000) {
                    // 1分钟内不重复搜索
        ...
        try {
            ...
            if (window.ede.cacheEnabled) {
                ...
                window.localStorage.setItem(
                    `_search_cache_${encodeURIComponent(name)}`,
                    JSON.stringify({
                        timestamp: Date.now(),
                        data: animaInfo,
                    }),
    ...
    `;
        ...
        // 获取已保存的过滤类型
        const selectedTypes = window.localStorage.getItem('danmakuTypeFilter') ? JSON.parse(window.localStorage.getItem('danmakuTypeFilter')) : [];
        ...
        `;
            ...
            checkbox.querySelector('input').addEventListener('change', (e) => {
                ...
                let types = window.localStorage.getItem('danmakuTypeFilter') ? JSON.parse(window.localStorage.getItem('danmakuTypeFilter')) : [];
    ...
    `;
        ...
        function setupToggle(toggleBtn, content, icon) {
            toggleBtn.onclick = () => {
                ...
                toggleBtn.querySelector('.md-icon:last-child').textContent = isExpanded ? 'expand_more' : 'expand_less';
            };
        }
        ...
        dialog.querySelector('#clearCacheBtn').onclick = () => {
            ...
            for (const key of keys) {
                if (key.startsWith('_danmaku_cache_') || key.startsWith('_search_cache_') || key.startsWith('_search_lock_')) {
                    localStorage.removeItem(key);
                    count++;
                }
            }
    ...
    function generateLogContent() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key.startsWith('_danmaku_cache_') || key.startsWith('_search_cache_')) {
      ...
      `;
        ...
        `;
                wordChip.querySelector('.removeWord').onclick = () => {
                    ...
                    window.localStorage.setItem('danmakuFilterWords', JSON.stringify(window.ede.filterWords));
